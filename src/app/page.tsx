...
export default function FinTrackPage() { 
  ...
  const addTransaction = async (values: TransactionFormValues, type: 'income' | 'expense') => {
    ...
    try {
      ...
      // To ensure date is a Date object for immediate UI update:
      const displayTransaction: Transaction = { 
        ...
        date: values.date // values.date is already a Date object from the form
      };
      setTransactions(prev => [displayTransaction, ...prev]);
    } catch (error) {
  ...
  const monthlyChartData = useMemo((): MonthlyData[] => {
    ...
    transactions.forEach(t => {
      ...
      if (isNaN(transactionDate.getTime())) {
        console.warn("Invalid date found for transaction:", t);
        return; // Skip this transaction
      }
      const monthYearKey = format(transactionDate, "MMM yyyy");
